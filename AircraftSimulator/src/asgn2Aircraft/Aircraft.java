...
public abstract class Aircraft {
	...
	protected String AircraftException;
	...
	public Aircraft(String flightCode,int departureTime, int first, int business, int premium, int economy) throws AircraftException { 
		if(
				...
				premium < 0 || 
				economy < 0 ) {
			throw new AircraftException("Invalid Input");
		}
	...
	public void cancelBooking(Passenger p,int cancellationTime) throws PassengerException, AircraftException {
		if(p.isConfirmed() == false) {
			throw new AircraftException("Is not confirmed");
		}
		if(cancellationTime <= 0) {
			throw new AircraftException("Invalud cancellation time must be greater than 0");
		}
		if(hasPassenger(p) == false ) {
			throw new AircraftException("Passenger not on flight");
		}	
		...
		if(p instanceof First){
			...
			numEconomy = numEconomy - 1;
		}		
		else {
			throw new PassengerException("Invalid passenger class");
		}
	}
	...
	public void confirmBooking(Passenger p,int confirmationTime) throws AircraftException, PassengerException { 
		if(
				...
				p.isFlown() == true ||
				p.isRefused() == true ||
				p.getDepartureTime() <= 0 ||
				confirmationTime < 0 ) {
			throw new PassengerException("Passenger Invalid");
		}
		if(checkClassCap(p) != true) {
			throw new AircraftException("No seats in fare class");
		}
		p.confirmSeat(confirmationTime, this.departureTime);
		this.status += Log.setPassengerMsg(p,"N/Q","C");
		if(this.seats.add(p) == false) {
			throw new AircraftException("Coudln't add passenger to seats");
		}
		if(p instanceof First){
			numFirst++;
		}
	...
	public boolean flightEmpty() {
		if(this.numBusiness + this.numEconomy + this.numFirst + this.numPremium == 0){
			return true;
		}
		else {
			return false;
	...
	public boolean flightFull() {
		if(this.numBusiness + this.numEconomy + this.numFirst + this.numPremium == this.capacity) {
			return true;
		}
		else {
			return false;
	...
	public void flyPassengers(int departureTime) throws PassengerException { 		
		for(int i = 0; i < this.seats.size(); i++) {
			if(this.seats.get(i).isConfirmed() != true) {
	...
	public Bookings getBookings() {
		int total = totalPassengers();
		int avaliable = totalAvaliable();
		Bookings bookingStatus = new Bookings(this.numFirst, this.numBusiness, this.numPremium, this.numEconomy,total,avaliable);
	...
	public int getNumPassengers() {
		return this.numBusiness + this.numEconomy + this.numFirst + this.numPremium;
	}
	...
	public List<Passenger> getPassengers() {
		List<Passenger> seatsDuplicate = new ArrayList<Passenger>(this.seats);
		return seatsDuplicate;
	}
	...
	public boolean hasPassenger(Passenger p) {
		boolean test = this.seats.contains(p);
		return test;
	}
	...
	//Used to get the passenger class
	/**
	 * Helper method to return the class letter for the input passenger
	 * @param p The Passenger we need to find out what there class is
	 * @return string with class letter
	 */
	private String passengerClass(Passenger p) {
		String pID = "";
		String flightClass = "";
		pID = p.getPassID();
		flightClass = pID.substring(0, 1);
		return flightClass;
	}
	
	//Used to get check the input class to see if it is full
	/**
	 * Helper method to return weather or not the class has space or not
	 * @param letter Single letter representing the class
	 * @return If there is space available or not for the specified class 
	 */
	private Boolean checkClassCap(Passenger p) {
		if(p instanceof First){ //First Class
			if(numFirst < firstCapacity) {
				return true;
			}
			else { return false; }
		}
		else if(p instanceof Business) { //Business Class
			if(numBusiness < businessCapacity) {
				return true;
			}
			else { return false; }
		}
		else if(p instanceof Premium) { //Premium Class
			if(numPremium < premiumCapacity) {
				return true;
			}
			else { return false; }
		}
		else if(p instanceof Economy) { //Economy Class
	...
	//Used to calculate the total number of passengers
	/**
	 * Helper method to calculate the total number of passengers across all fair classes
	 * @return total number of passengers
	 */
	private int totalPassengers() {
		int total = 0;
		total = numFirst + numBusiness + numPremium + numEconomy;
		return total;
	}
	
	//Use to calculate the total number of seats available
	/**
	 * Helper method to calculate the total number of available seats on the plan
	 * @return total number of seats available on the plan
	 */
	private int totalAvaliable() {
		int maxCapacity = 0;
		int currentNumPassengers = 0;
		int totalAvaliable = 0;
		maxCapacity = firstCapacity + businessCapacity + premiumCapacity + economyCapacity;
		currentNumPassengers = totalPassengers();
		totalAvaliable = maxCapacity - currentNumPassengers;
