...
public abstract class Passenger {
	...
	public void cancelSeat(int cancellationTime) throws PassengerException {
		if ( 
			...
			throw new PassengerException ("Invalid state or vierable input"); 
		}
	...
	public void confirmSeat(int confirmationTime, int departureTime) throws PassengerException {	
		if(this.newState == false && this.isQueued() == false) {
			throw new PassengerException("Invalid state, must be new or queued");
		}
		if ( 
		...
		this.bookingTime = confirmationTime;
		this.departureTime = departureTime; 		
		if(this.isNew()) 
		{
			this.newState = false;
	...
	public void flyPassenger(int departureTime) throws PassengerException {
		if ( 
				...
				this.isFlown() == true || 
				departureTime <= 0 ) {
			throw new PassengerException ("invalid state"); 
		}	
	...
	public void refusePassenger(int refusalTime) throws PassengerException {
		if( this.isNew() == false && this.isQueued() == false) {
			throw new PassengerException("Invalid state");
		}
		if ( 
				this.isConfirmed() == true || 
